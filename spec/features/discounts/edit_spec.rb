...
RSpec.describe 'Merchant Bulk Discount Edit' do
  ...
  describe 'User Story 6' do
    it 'Index page has a link to edit discount' do
    ...
    it 'changes information, clicks submit and redirecteds to show page with changes present' do
      ...
      fill_in "discount_name", with: "Drogon's Deal"
      click_on "Edit #{@discount1.name} Discount"
      expect(current_path).to eq(merchant_discount_path(@merchant1, @discount1))

      expect(page).to have_content("Drogon's Deal Has Been Updated!")
      expect(page).to have_content("Drogon's Deal")
